...
export default function KycVerificationPage() {
  ...
  // Check if user is a buyer and needs to upgrade to seller (Flow 2)
  useEffect(() => {
    if (!isProfileLoading && user) {
      const isBuyer =
        user.roles &&
        !user.roles.includes("seller") &&
        (user.roles.includes("buyer") || user.roles.length === 0);

      if (isBuyer) {
        // Flow 2: Buyer wants to become a seller - upgrade role first
  ...
  }, [user, isProfileLoading, upgradeToSeller]);
  ...
  if (isProfileLoading || isCheckingRole || isUpgrading) {
    return (
      <main className="p-6 lg:p-8">
        <div className="max-w-2xl mx-auto">
          <div className="bg-white rounded-xl border border-[#e7e8e9] p-8 text-center">
            <div className="flex justify-center mb-6">
              <Loader2 className="w-12 h-12 animate-spin text-[#f10e7c]" />
            </div>
            <h2 className="text-2xl font-bold text-[#1d1d2a] mb-4">
              {isUpgrading ? "Upgrading your account..." : "Loading..."}
            </h2>
            <p className="text-[#667185]">
              {isUpgrading
                ? "Please wait while we upgrade your account to seller."
                : "Please wait..."}
            </p>
          </div>
        </div>
        ...
        <div className="max-w-2xl mx-auto">
          <div className="bg-white rounded-xl border border-[#e7e8e9] p-8 text-center">
            ...
            <p className="text-[#667185] mb-6">
              ...
              successfully. You can now proceed to create your shop.
            </p>
            <div className="flex justify-center">
              <Loader2 className="w-5 h-5 animate-spin text-[#f10e7c]" />
              <span className="ml-2 text-[#667185]">
                Redirecting to create shop...
              </span>
            </div>
          </div>
        </div>
    ...
    <main className="p-6 lg:p-8">
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-[14px] text-[#667185] leading-relaxed">
              Verify your identity to become a seller
            </p>
          </div>
        </div>
      </div>
      ...
      <div className="max-w-2xl mx-auto">
        <div className="bg-white rounded-xl border border-[#e7e8e9] p-8">
          <div className="mb-6">
            <h2 className="text-[20px] font-semibold text-[#1d1d2a] mb-3">
              National Identification Number (NIN)
            </h2>
            <p className="text-[14px] text-[#667185] leading-relaxed">
              Please enter your 11-digit National Identification Number (NIN) to
              verify your identity. This is required to create a shop on our
              platform.
            </p>
          </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            <div>
              ...
              <p className="text-[12px] text-[#667185] mt-2">
                Your NIN will be securely verified with the National Identity
                Management Commission (NIMC).
              </p>
            </div>

            <div className="bg-[#f9fafb] rounded-lg p-4 border border-[#e7e8e9]">
              <h3 className="text-[14px] font-semibold text-[#1d1d2a] mb-2">
