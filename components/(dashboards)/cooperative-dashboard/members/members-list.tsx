    ...
    <Card className="border-[#e4e7ec] bg-white">
      <CardHeader className="border-b border-[#e4e7ec] px-4 py-4 sm:px-6">
        <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
          <CardTitle className="text-lg font-semibold text-[#1d1d2a]">Member List</CardTitle>
          <div className="flex flex-col gap-3 sm:flex-row sm:items-center">
            ...
            <Button variant="outline" className="border-[#e4e7ec] bg-white hover:bg-[#f5f5f5]">
        ...
        <div className="hidden overflow-x-auto lg:block">
          <table className="w-full">
            <thead className="border-b border-[#e4e7ec] bg-[#f9fafb]">
              <tr>
                <th className="px-6 py-4 text-left text-xs font-medium text-[#838794]">DAW-ID</th>
                <th className="px-6 py-4 text-left text-xs font-medium text-[#838794]">Name</th>
                <th className="px-6 py-4 text-left text-xs font-medium text-[#838794]">Email</th>
                <th className="px-6 py-4 text-left text-xs font-medium text-[#838794]">Role</th>
                <th className="px-6 py-4 text-left text-xs font-medium text-[#838794]">Reg. Date</th>
                <th className="px-6 py-4 text-left text-xs font-medium text-[#838794]">Total Sales</th>
            ...
            <tbody className="divide-y divide-[#e4e7ec]">
              {members.map((member, index) => (
                <tr key={index} className="hover:bg-[#f9fafb]">
                  <td className="px-6 py-4 text-sm text-[#1d1d2a]">{member.id}</td>
                  <td className="px-6 py-4">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        ...
                        {member.description && <p className="text-xs text-[#838794]">{member.description}</p>}
                      </div>
                    </div>
                  </td>
                  <td className="px-6 py-4">
                    <div>
                      ...
                      {member.emailSecondary && <p className="text-xs text-[#838794]">{member.emailSecondary}</p>}
                    </div>
                  </td>
                  <td className="px-6 py-4 text-sm text-[#1d1d2a]">{member.role}</td>
                  <td className="px-6 py-4 text-sm text-[#1d1d2a]">{member.regDate}</td>
                  <td className="px-6 py-4 text-sm font-medium text-[#1d1d2a]">{member.totalSales}</td>
                  <td className="px-6 py-4">
                    <span
            ...
            <div key={index} className="p-4">
              <div className="flex items-start justify-between">
                <div className="flex items-start gap-3">
                  ...
                  <div className="flex-1">
                    <p className="text-sm font-medium text-[#1d1d2a]">{member.name}</p>
                    {member.description && <p className="text-xs text-[#838794]">{member.description}</p>}
                    <p className="mt-1 text-xs text-[#838794]">{member.email}</p>
                    {member.emailSecondary && <p className="text-xs text-[#838794]">{member.emailSecondary}</p>}
                  </div>
                </div>
                <DropdownMenu>
              ...
              <div className="mt-3 grid grid-cols-2 gap-2 text-xs">
                ...
                <div>
                  ...
                  <span className="font-medium text-[#1d1d2a]">{member.totalSales}</span>
                </div>
              </div>
              <div className="mt-3">
                <span
